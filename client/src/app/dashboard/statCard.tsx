...
const StatCard = ({
  ...
  const formatPercentage = (value: number) => {
    ...
    return `${signal}${Math.abs(value).toFixed()}%`;
  };
    ...
    <div className="md:row-span-1 xl:row-span-2 bg-white col-span-1 shadow-md rounded-2xl flex flex-col justify-between">
      ...
      <div>
        <div className="flex justify-between items-center mb-2 px-5 pt-4">
          <h2 className="font-semibold text-l text-gray-700">{t(title)}</h2>
          <span className="text-[0.80rem] text-gray-500">{dateRange}</span>
        </div>
        <hr />
      </div>
      ...
      <div className="flex mb-6 items-center justify-around gap-4 px-5">
        <div className="rounded-full p-2 bg-blue-50 border-sky-300 border-[1px]">
          {primaryIcon}
        </div>
